/**
 *Submitted for verification at BscScan.com on 2025-07-04
*/
...
/**
 * - NeoLea Token
 * - Maximum supply cap of 10 billion tokens
 * - Multi-role security
 */
contract NeoLea is ERC20, ERC20Burnable, ERC20Pausable, AccessControl {
    ...
    event TokensLocked(address indexed account, uint256 releaseTime);
    ...
    constructor(
        ...
        require(initialSupply <= MAX_SUPPLY / 10**18, "Initial supply exceeds maximum");
        require(initialAdmin != address(0), "Admin cannot be zero address");
        require(initialMinter != address(0), "Minter cannot be zero address");
        require(initialPauser != address(0), "Pauser cannot be zero address");
        require(
            initialAdmin != initialMinter && 
            initialAdmin != initialPauser && 
            initialMinter != initialPauser, 
    ...
    return _blacklisted[account];
       ...
       function lockTokens(address account, uint256 releaseTime) external {
        require(
        ...
        _timelocks[account] = releaseTime;
        emit TokensLocked(account, releaseTime);
    }
    
        function unlockTokens(address account) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
        require(_timelocks[account] > 0, "Not locked");
        
        _timelocks[account] = 0;
        emit TokensUnlocked(account);
    }
        
    function getTokenUnlockTime(address account) external view returns (uint256) {
    ...
    function _update(
        ...
        if (from != address(0) && to != address(0)) {
            require(
                _timelocks[from] == 0 || block.timestamp >= _timelocks[from],
                "Tokens locked"
            );
        }
